...
export default function StationsDatabasePage() {
  // 1. MOCK DATA (This would normally come from your DB)
  const [stations, setStations] = useState<Station[]>([
    { id: 1, name: "North Avenue", status: "OPEN", crowdLevel: "MODERATE" },
    { id: 2, name: "Quezon Avenue", status: "OPEN", crowdLevel: "LOW" },
    { id: 3, name: "GMA Kamuning", status: "MAINTENANCE", crowdLevel: "LOW" },
  ]);
  ...
  const handleCreate = (e: React.FormEvent) => {
    ...
    const newStation: Station = {
      ...
      name: newStationName,
      status: 'CLOSED', // Default to closed for safety
      crowdLevel: 'LOW'
    };
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Stations Database</h1>
          <p className="text-slate-500">Manage the 13 MRT-3 stations.</p>
        </div>
        ...
        <div className="mb-6 bg-blue-50 border border-blue-100 p-4 rounded-xl animate-in fade-in slide-in-from-top-4">
            <form onSubmit={handleCreate} className="flex gap-4 items-end">
                <div className="flex-1">
                    <label className="block text-xs font-semibold text-blue-900 mb-1">STATION NAME</label>
                    <input 
                        autoFocus
                        type="text" 
        ...
        <table className="w-full text-left border-collapse">
          ...
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Crowd</th>
